...
**Exit rules & portfolio Greeks — live dashboard.** The top panel shows
three stop-loss variants on the same picks: the real 2× stop (green,
steepest loss), a tight 1× stop (blue), and no stop at all (orange).
The bottom panel shows real broker-computed Greeks across all open
spreads — delta, gamma, theta, vega, rho — plus beta-weighted delta
in SPY-equivalent shares, broken down per position.

![Exit rule comparison and Portfolio Greeks — live dashboard](docs/assets/exit-rules-greeks.jpg)
