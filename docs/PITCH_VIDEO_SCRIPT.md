...
| **6. Why this is real** | 3:28–3:48 | Fast montage: repo file tree, the test suite running green, the dashboard, the `/compare` curves, a terminal `git log` | "To be blunt about the depth: **51 Python modules. MCP-native. Fourteen hard risk checks. A live dashboard, on the web and in Telegram. A hundred-case test suite. A shadow book against a mechanical rule and a coin flip. Three agents, one shared risk brain, compared in public.** This is a system, and it's all in the repo." |
